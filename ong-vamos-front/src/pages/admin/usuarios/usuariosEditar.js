...
export default function UsuariosCadastrar() {
  ...
  if (proje != null) {
    ...
    useEffect(() => {
      async function getAssistido() {
        var result = await api.get('/assistidos/' + idAssistido);
  
        const tel = `${result.data.telefone[0].ddd} ${result.data.telefone[0].numero}`
        setNome(result.data.nome);
        setCpf(result.data.cpf);
        setSexo(result.data.sexo);
        setTelefone(tel);
        setCep(result.data.enderecos[0].cep);
        setLogradouro(result.data.enderecos[0].logradouro);
        setCidade(result.data.enderecos[0].cidade)
        setEstado(result.data.enderecos[0].estado)
        setBairro(result.data.enderecos[0].bairro)
        setNumero(result.data.enderecos[0].numero)
      }
      getAssistido();
    }, [idAssistido]);
  ...
  const handleSubmit = useCallback(
    (event) => {
      ...
      api.put(`assistidos/${idAssistido}`, {
        assistido: {
          nome,
          nascimento,
          cpf,
        ...
        <main className={classes.main}>
          <div className={classes.container}>
            ...
            <div className={classes.content}>
              <Grid>
                <div>
                  <h1 style={{ textAlign: "center" }}>Editar Assistido</h1>
                </div>
